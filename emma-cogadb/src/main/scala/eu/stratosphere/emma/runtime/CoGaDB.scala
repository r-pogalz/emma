...
import emma.ir._
import eu.stratosphere.emma.codegen.cogadb.{DataflowGenerator, TempResultsManager}
 ...
 *
 * @param env The backing CoGaDB execution environment.
 */
class CoGaDB(val env: ExecutionEnvironment) extends Engine {
...
object CoGaDB extends Engine.Factory {

  def apply(env: ExecutionEnvironment): CoGaDB =
    new CoGaDB(env)

  override def default(): CoGaDB =
    new CoGaDB

  override def testing(): CoGaDB = {
