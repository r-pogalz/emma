...
//    TermName("$up") -> '^,
  ...
  def newMapUDFOutput(tpe: Type)(implicit outputs: ListBuffer[UDFOutput]): TypeName = newUDFOutput(tpe, "MAP_UDF_RES_")
  
  def newUDFOutput(tpe: Type, prefix: String)(implicit outputs: ListBuffer[UDFOutput]): TypeName = {
    val name = freshTypeName(prefix)
    outputs += new UDFOutput(name, tpe)
    name
  }
