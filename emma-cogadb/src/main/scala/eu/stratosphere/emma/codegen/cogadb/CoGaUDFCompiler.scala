...
object CoGaUDFCompiler {
  ...
  private def parseEntry(tree: Tree)
  ...
  private def parse(tree: Tree,
                    potentialOutput: Boolean)
                   (implicit closure: UDFClosure,
                    outputs: ListBuffer[CoGaUDFOutput]): String = {
    tree match {
      case Block(stats,
                 expr) => parse(stats) + parse(expr,
                                               true)
      case app@Apply(fun: Select,
                     args: List[Tree]) => {
        if (potentialOutput) {
          if ("apply".equals(fun.name.toString)) {
            ...
            val stmtBlock = args.map(arg => {
              ...
              outputs += new CoGaUDFOutput(outputIde,
                                           arg.tpe)
              toCode(CoGaExpr.Assignment,
                     toCode(CoGaExpr.CoGaOutputCol,
                            outputIde),
                     parseExpr(arg))
            })
            stmtBlock.mkString
          ...
          if ("apply".equals(fun.name.toString)) {
            //complex type init
            val stmtBlock = args.map(arg => toCode(CoGaExpr.NoOp,
                                                   parseExpr(app)))
            stmtBlock.mkString
          } else {
            //single stmt
            toCode(CoGaExpr.NoOp,
                   parseExpr(app))
          }
        }
      }
      case app@Apply(fun: TypeApply,
                     args: List[Tree]) => {
      ...
      case If(cond,
              thenp,
              elsep) => elsep match {
        case Literal(_) => toCode(CoGaExpr.IfThen,
                                  parseExpr(cond),
                                  parse(thenp,
                                        false))
        case _ => toCode(CoGaExpr.IfThenElse,
                         parseExpr(cond),
                         parse(thenp,
                               false),
                         parse(elsep,
  ...
  private def parse(trees: List[Tree])
    ...
    if (trees.isEmpty) {
      ...
      parse(trees.head,
            false) + parse(trees.tail)
    }
  }

  private def parseExpr(tree: Tree)
                       (implicit closure: UDFClosure,
                        outputs: ListBuffer[CoGaUDFOutput]): String = {
    println("----------------- Raw tree ---------------------")
    println(showRaw(tree))
    ...
    tree match {
      case Apply(fun: Select,
                 args: List[Tree]) => parseApply(fun,
                                                 args)
      case sel@Select(qualifier,
                      name) => parseSelect(sel)
  ...
  private def parseApply(fun: Select,
                         args: List[Tree])
                        (implicit closure: UDFClosure,
                         outputs: ListBuffer[CoGaUDFOutput]): String = {
    if (fun.name.toString.startsWith("$")) {
      args.size match {
        case 1 => toCode(CoGaExpr.BinaryOperation,
                         BOperator.fromScalaOp(fun,
                                               fun.tpe.finalResultType.toString),
                         parseExpr(fun.qualifier),
                         parseExpr(args.head))
        case _ => throw new IllegalArgumentException(s"Apply with args size not supported: ${args.size}")
      }
    } else {
      args.size match {
        case 1 => toCode(CoGaExpr.UnaryOperation,
                         UOperator.fromScalaOp(fun),
                         parseExpr(args(0)))
        case 2 => toCode(CoGaExpr.BinaryOperation,
                         BOperator.fromScalaOp(fun,
                                               fun.tpe.finalResultType.toString),
                         parseExpr(args(0)),
                         parseExpr(args(1)))
        case _ => throw new IllegalArgumentException(
                                                      s"Apply with args size ${args.size} for function ${
                                                        fun.name.toString
                                                      } not supported.")
      }
    }
  ...
  private def parseSelect(sel: Select)
                         (implicit closure: UDFClosure,
                          outputs: ListBuffer[CoGaUDFOutput]): String = {
    if (isCast(sel.name)) {
      sel.qualifier match {
        case ide@Ident(_) => toCode(CoGaExpr.UnaryOperation,
                                    UOperator.fromScalaOp(sel),
                                    parseExpr(ide))
  ...
  private def generateVar(ide: Ident): String = toCode(CoGaExpr.LocalVar,
                                                       ide.name.toString)

  private def generateVarDef(name: Name,
                             tpt: TypeTree): String =
    toCode(CoGaExpr.VarDef,
           Type.fromScalaType(TypeExtractor.extractNameFrom(tpt)).toString,
           name.toString)

  private def generateConst(lit: Literal): String =
    toCode(CoGaExpr.Const,
           lit.value.tpe.toString,
